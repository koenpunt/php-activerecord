...
class ActiveRecordWriteTest extends DatabaseTest
{
	...
	/**
	 * @todo this is a useless test because mysql doesn't support sequences,
	 *  but the connection used is a mysql connection
	...
	public function test_insert_should_quote_keys()
	{
		$author = new Author(array('name' => 'Blah Blah'));
		$author->save();
		$this->assertContains($author->connection()->quote_name('updated_at'), $author->connection()->last_query);
	}
	...
	public function test_update_should_quote_keys()
	{
		...
		$book->name = 'new name';
		$book->save();
		$this->assertContains($book->connection()->quote_name('name'),$book->connection()->last_query);
	}
	...
	public function test_dirty_attributes_cleared_after_saving()
	{
		...
		$this->assertContains('name',$book->table()->last_sql);
		$this->assertContains('special',$book->table()->last_sql);
	...
	public function test_delete_all_with_limit_and_order()
	{
		...
		$this->assertContains('ORDER BY name asc LIMIT 1',Author::table()->last_sql);
	}
