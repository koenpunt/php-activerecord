...
class SQLBuilderTest extends DatabaseTest
{
	...
	public function assert_conditions($expected_sql, $values, $underscored_string, $map=null)
	{
		...
		$this->assertSqlContains($expected_sql,array_shift($cond));
	...
	public function test_where_with_array()
	{
		...
		$this->assertSqlContains("SELECT * FROM authors WHERE id=? AND name IN(?,?)",(string)$this->sql);
	...
	public function test_where_with_hash()
	{
		...
		$this->assertSqlContains("SELECT * FROM authors WHERE id=? AND name=?",(string)$this->sql);
	...
	public function test_gh134_where_with_hash_and_null()
	{
		...
		$this->assertSqlContains("SELECT * FROM authors WHERE id=? AND name IS ?",(string)$this->sql);
	...
	public function test_all_clauses_after_where_should_be_correctly_ordered()
	{
		...
		$this->assertSqlContains($this->conn->limit("SELECT * FROM authors WHERE id=? GROUP BY name HAVING created_at > '2009-01-01' ORDER BY name",1,10), (string)$this->sql);
	}
	...
	public function test_insert()
	{
		...
		$this->assertSqlContains("INSERT INTO authors(id,name) VALUES(?,?)",(string)$this->sql);
	}

	public function test_insert_with_null()
	{
		$this->sql->insert(array('id' => 1, 'name' => null));
		$this->assertSqlContains("INSERT INTO authors(id,name) VALUES(?,?)",$this->sql->to_s());
	}

	public function test_update_with_hash()
	{
		$this->sql->update(array('id' => 1, 'name' => 'Tito'))->where('id=1 AND name IN(?)',array('Tito','Mexican'));
 		$this->assertSqlContains("UPDATE authors SET id=?, name=? WHERE id=1 AND name IN(?,?)",(string)$this->sql);
	...
	public function test_update_with_limit_and_order()
	{
		...
		$this->assertSqlContains("UPDATE authors SET id=? ORDER BY name asc LIMIT 1", $this->sql->to_s());
	}

	public function test_update_with_string()
	{
		$this->sql->update("name='Bob'");
		$this->assertSqlContains("UPDATE authors SET name='Bob'", $this->sql->to_s());
	}

	public function test_update_with_null()
	{
		$this->sql->update(array('id' => 1, 'name' => null))->where('id=1');
		$this->assertSqlContains("UPDATE authors SET id=?, name=? WHERE id=1",$this->sql->to_s());
	}
	...
	public function test_delete_with_hash()
	{
		...
		$this->assertSqlContains("DELETE FROM authors WHERE id=? AND name IN(?,?)",$this->sql->to_s());
		$this->assertEquals(array(1,'Tito','Mexican'),$this->sql->get_where_values());
	}
	...
	public function test_delete_with_limit_and_order()
	{
		...
		$this->assertSqlContains("DELETE FROM authors WHERE id=? ORDER BY name asc LIMIT 1",$this->sql->to_s());
	}
	...
	public function test_where_with_joins_prepends_table_name_to_fields()
	{
		...
		$this->assertSqlContains("SELECT * FROM authors $joins WHERE authors.id=? AND authors.name=?",(string)$this->sql);
	}
}
