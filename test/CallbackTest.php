...
class CallBackTest extends DatabaseTest
{
	...
	public function assert_has_callback($callback_name, $method_name=null)
	{
		...
		$this->assertContains($method_name,$this->callback->get_callbacks($callback_name));
	}
	...
	/**
	 * @expectedException ActiveRecord\ActiveRecordException
	 * @expectedExceptionMessage Unknown method for callback
	 */
	...
	{
		$this->callback->register('after_construct');
		$this->assertInternalType('array', $this->callback->get_callbacks('after_construct'));
	}
	...
	/**
	 * @expectedException ActiveRecord\ActiveRecordException
	 * @expectedExceptionMessage No callbacks were defined for
	 */
	...
	public function test_before_create_returned_false_halts_execution()
	{
		...
		$i_ran = false;
		$i_should_have_run = false;
		$table->callback->register('before_save', function($model) use (&$i_should_have_run) { $i_should_have_run = true; });
		...
		$this->assertTrue($i_should_have_run);
		$this->assertFalse($i_ran);
		$this->assertNotContains('INSERT', ActiveRecord\Table::load('VenueCB')->last_sql);
	}
	...
	public function test_before_save_returned_false_halts_execution()
	{
		...
		$this->assertNotContains('UPDATE', ActiveRecord\Table::load('VenueCB')->last_sql);
	}
	...
	public function test_before_destroy_returned_false_halts_execution()
	{
		...
		$this->assertFalse($i_ran);
		$this->assertFalse($ret);
		$this->assertNotContains('DELETE', ActiveRecord\Table::load('VenueCB')->last_sql);
	}
