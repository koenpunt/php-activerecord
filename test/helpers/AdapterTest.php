...
class AdapterTest extends DatabaseTest
{
	...
	public function test_date_time_type()
	{
		...
		$this->assertNotEmpty($columns['created_at']->length);
	}
	...
	public function test_date()
	{
		...
		$this->assertGreaterThanOrEqual(7, $columns['some_Date']->length);
	}

	public function test_columns_no_inflection_on_hash_key()
	{
		$author_columns = $this->conn->columns('authors');
		$this->assertArrayHasKey('author_id', $author_columns);
	}
	...
	{
		$author_columns = $this->conn->columns('authors');
		$this->assertStringStartsWith('varchar',$author_columns['name']->raw_type);
	...
	public function test_columns_decimal()
	{
		...
		$this->assertGreaterThanOrEqual(10, $columns['special']->length);
	}
	...
	{
		$this->assertGreaterThan(0,count($this->conn->tables()));
		$this->assertCountGreaterThan(0,$this->conn->tables());
	}

	/**
	 * @todo query_column_info returns a PDOStatement, so what does asserting `count` proves here?
	 */
	...
	/**
	 * @todo query_for_tables returns a PDOStatement, so what does asserting `count` proves here?
	 */
	...
	/**
	 * @expectedException ActiveRecord\DatabaseException
	 * @expectedExceptionMessageRegExp (an_invalid_column)|(exist)
	 */
