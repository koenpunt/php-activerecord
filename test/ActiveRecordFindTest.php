...
class ActiveRecordFindTest extends DatabaseTest
{
	...
	/**
	 * @expectedException ActiveRecord\RecordNotFound
	 * @expectedExceptionMessage found 1, but was looking for 2
	 */
	...
	public function test_find_by_pk_with_options()
	{
		...
		$this->assertEquals(3,$author->id);
		$this->assertContains('ORDER BY name', Author::table()->last_sql);
	}
	...
	public function test_find_all()
	{
		...
		$this->assertCountGreaterThanOrEqual(3, $authors);
	}
	 ...
	 * @expectedExceptionMessage no parameters were bound
	 */
	...
	public function test_find_hash_using_alias()
	{
		...
		$this->assertCountGreaterThanOrEqual(1, $venues);
	}
	...
	public function test_find_all_hash_with_order()
	{
		...
		$this->assertCountGreaterThan(0, $books);
	}

	public function test_find_all_no_args()
	{
		$author = Author::all();
		$this->assertCountGreaterThan(1, $author);
	}
	...
	public function test_for_each()
	{
		...
		{
			$this->assertInstanceOf('ActiveRecord\Model', $author);
			$i++;
		}
	...
	public function test_joins_on_model_with_association_and_explicit_joins()
	{
		...
		$this->assertSqlContains('INNER JOIN authors ON(books.author_id = authors.author_id)',JoinBook::table()->last_sql);
		$this->assertSqlContains('LEFT JOIN authors a ON(books.secondary_author_id=a.author_id)',JoinBook::table()->last_sql);
	}
	...
	public function test_group()
	{
		...
		$this->assertGreaterThan(0,$venues);
		$this->assertSqlContains('GROUP BY state',ActiveRecord\Table::load('Venue')->last_sql);
	}
	...
	public function test_group_with_order_and_limit_and_having()
	{
		...
		$this->assertGreaterThan(0,$venues);
		$this->assertSqlContains($this->conn->limit('SELECT state FROM venues GROUP BY state HAVING length(state) = 2 ORDER BY state',null,2),Venue::table()->last_sql);
	}
	...
	public function test_having()
	{
		if ($this->conn instanceof ActiveRecord\OciAdapter)
		{
			...
			$this->assertSqlContains("GROUP BY to_char(created_at,'YYYY-MM-DD') HAVING to_char(created_at,'YYYY-MM-DD') > '2009-01-01'",Author::table()->last_sql);
		}
		else
		{
			$author = Author::first(array(
			...
			$this->assertSqlContains("GROUP BY date(created_at) HAVING date(created_at) > '2009-01-01'",Author::table()->last_sql);
		}
	}
	...
	public function test_find_with_hash()
	{
		...
		$this->assertCount(1,Author::find('all',array('name' => 'Tito')));
		$this->assertCount(1,Author::all(array('name' => 'Tito')));
	}
	...
	public function test_find_or_create_by_creates_new_record()
	{
		...
		$this->assertNotEmpty($author->author_id);
		$this->assertEquals('pencil',$author->encrypted_password);
	}

	/**
	 * @expectedException ActiveRecord\ActiveRecordException
	 * @expectedExceptionMessage Cannot use OR'd attributes in find_or_create_by
	 */
	...
	{
		Author::find(1);
		$this->assertSqlContains('SELECT * FROM authors WHERE author_id=?',Author::table()->last_sql);
	}
