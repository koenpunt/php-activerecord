...
class SerializationTest extends DatabaseTest
{
	...
	public function test_to_json()
	{
		$book = Book::find(1);
		$json = $book->to_json();
		$this->assertEquals($book->attributes(), json_decode($json, true));
	}
	...
	{
		$this->assertStringStartsNotWith('<?xml version', Book::find(1)->to_xml(array('skip_instruct' => true)));
		$this->assertStringStartsWith('<?xml version', Book::find(1)->to_xml(array('skip_instruct' => false)));
	}
